...
do
  num=`printf "%02d\n" $i`
  spark-sql ${MASTER_OPTIONS} ${DRIVER_OPTIONS} ${EXECUTOR_OPTIONS} ${ADDITION_SPARK_OPTIONS} -database TPCDS -f ${OUTPUT_DIR}/query${num}.sql > ${OUTPUT_DIR}/query${num}.res 2>&1 
